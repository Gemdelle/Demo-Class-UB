...
class FatherGame1:
    ...
    def create_objects(self):
        self.objects['crystal1'] = self.tile_map.create_oval(
            self.get_centered_coords(3, 0, offset=self.tile_size // 4), fill='orange')
        self.objects['crystal2'] = self.tile_map.create_oval(
            self.get_centered_coords(1, 1, offset=self.tile_size // 4), fill='grey')
        self.objects['crystal3'] = self.tile_map.create_oval(
            self.get_centered_coords(1, 2, offset=self.tile_size // 4), fill='red')
        self.objects['crystal4'] = self.tile_map.create_oval(
            self.get_centered_coords(3, 2, offset=self.tile_size // 4), fill='blue')
        self.objects['food'] = self.tile_map.create_oval(
            self.get_centered_coords(1, 0, offset=self.tile_size // 4), fill='black')
        self.objects['father'] = self.tile_map.create_oval(
            self.get_centered_coords(4, 4, offset=self.tile_size // 4), fill='white')

    def create_objects_2(self):
        self.objects['crystal1'] = self.tile_map.create_oval(
            self.get_centered_coords(4, 0, offset=self.tile_size // 4), fill='orange')
        self.objects['crystal2'] = self.tile_map.create_oval(
            self.get_centered_coords(2, 1, offset=self.tile_size // 4), fill='grey')
        self.objects['crystal3'] = self.tile_map.create_oval(
            self.get_centered_coords(4, 2, offset=self.tile_size // 4), fill='red')
        self.objects['crystal4'] = self.tile_map.create_oval(
            self.get_centered_coords(1, 3, offset=self.tile_size // 4), fill='blue')
        self.objects['food'] = self.tile_map.create_oval(
            self.get_centered_coords(4, 1, offset=self.tile_size // 4), fill='black')
        self.objects['father'] = self.tile_map.create_oval(
            self.get_centered_coords(0, 4, offset=self.tile_size // 4), fill='white')

    def create_blue_bar(self):
        # Create a blue bar on the tile_map
        self.blue_bar = self.tile_map.create_rectangle(
            10, 10, self.tile_size * self.grid_size, 20, fill="blue", outline="")
        # Start decrementing the bar's width
        self.decrement_blue_bar(self.steps)

    def decrement_blue_bar(self, steps):
        if steps >= 0:
            current_coords = self.tile_map.coords(self.blue_bar)
            new_width = current_coords[2] - (self.tile_size * self.grid_size) / 7
            self.tile_map.coords(self.blue_bar, current_coords[0], current_coords[1], new_width, current_coords[3])
        else:
            self.has_lost = True
            self.tile_map.delete(self.blue_bar)
            self.check_reset_positions()
